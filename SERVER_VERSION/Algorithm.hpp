...
public:
	 ...
	 * algorithm
	 */
	std::uint16_t runAlgorithm(const std::vector<Box>& boxes) const;
	 ...
	 */
	Graph::BipartiteGraph<Box> createGraphFromBoxes(const std::vector<Box>& boxes) const;
...
private:
	 ...
	 */
	bool kuhn(std::uint16_t vertex) const;
...
private:
	 ...
	 * 		  of a chain
	 */
	mutable std::vector<std::uint16_t> pairs;
};
